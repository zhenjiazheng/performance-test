...
class UserScanAddCart_User extends Simulation {
...
//  println("Data Source Length:" + feeder2.records.length)
  ...
  object addCartTest {
    val test = scenario("user_addCart") // A scenario is a chain of requests and pauses
      .feed(feeder)
      .feed(feeder2)
...
//            }
  ...
  val user_addCart = scenario("user_addCart").exec(addCartTest.test)
  setUp(
    //    notice.inject(constantUsersPerSec(1) during(10)),
    user_addCart.inject(constantUsersPerSec(70) during (3600))
  ).protocols(httpConf)
}
