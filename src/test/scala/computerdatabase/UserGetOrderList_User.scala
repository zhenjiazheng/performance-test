...
class UserGetOrderList_User extends Simulation {
...
//  println("Data Source Length:" + feeder.records.length)
  ...
  val feeder2 = jdbcFeeder("jdbc:mysql://172.16.0.52:5306/product", "postest", "DJdg@903902901", "SELECT  product.id as pid, product_spec.id as specId, product.user_id as store_id FROM product, product_spec WHERE product.user_id = 10 And product_spec.product_id = product.id ").circular
//  println(feeder2)
...
//  println("Data Source Length:" + feeder2.records.length)
  ...
  object getOrderListTest {
    val test = scenario("getOrderList") // A scenario is a chain of requests and pauses
      .feed(feeder)
      .feed(feeder2)
...
//      )
      .doIf(session => session.contains("token_w")) {
          exec(http("clear").post("ja/user/v1/cart/10/clear")
            ...
            .doIf(session => session.contains("pid") && session.contains("specId")) {
                ...
                .exec(http("order_commit").post("ja/user/v1/order/commit")
                  .headers(Map("token" -> "${token_w}"))
                  .body(StringBody("""{"table":"5", "useBalance":false, "storeId":${store_id}, "saleType": 1}"""))
                  .asJSON.check(status.is(200) ,jsonPath("$.statusCode").is(0.toString()))
                )

                .exec(http("getOrderList").get("ja/user/v1/order/1/list?storeNo=${store_id}&page=1&limit=20")
                  .headers(Map("token" -> "${token_w}"))
  ...
  val getOrderList = scenario("getOrderList").exec(getOrderListTest.test)
  setUp(
    //    notice.inject(constantUsersPerSec(1) during(10)),
    getOrderList.inject(constantUsersPerSec(10) during (10))
  ).protocols(httpConf)
}
