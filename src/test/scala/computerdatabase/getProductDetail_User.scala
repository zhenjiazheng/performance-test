...
class getProductDetail_User extends Simulation {
...
//  println("Data Source Length:" + feeder.records.length)
  ...
  val feeder2 = jdbcFeeder("jdbc:mysql://172.16.0.52:5306/product", "postest", "DJdg@903902901", "SELECT  product.id as pid FROM product, product_spec WHERE product.user_id = 10 And product_spec.product_id = product.id ").circular
...
//            }
            .doIf(session => session.contains("token_w")) {
              exec(http("getProductDetail").get("ja/user/v1/store/product/${pid}")
                .headers(Map("token" -> "${token_w}"))
...
//        }
  ...
  val getProductDetail = scenario("getProductDetail").exec(getProductDetailTest.test)
  setUp(
    //    notice.inject(constantUsersPerSec(1) during(10)),
    getProductDetail.inject(constantUsersPerSec(20) during (200))
  ).protocols(httpConf)
}
