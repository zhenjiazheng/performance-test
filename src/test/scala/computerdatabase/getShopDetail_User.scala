...
class getShopDetail_User extends Simulation {
...
//  println("Data Source Length:" + feeder2.records.length)
  ...
  object getShopDetailTest {
    val test = scenario("getShopDetail") // A scenario is a chain of requests and pauses
      .feed(feeder)
      .feed(feeder2)
...
//            }
            .doIf(session => session.contains("token_w")) {
              exec(http("getShopDetail").get("ja/user/v1/store/${storeId}")
                .headers(Map("token" -> "${token_w}"))
...
//        }
  ...
  val getShopDetail = scenario("getProductList").exec(getShopDetailTest.test)
  setUp(
    //    notice.inject(constantUsersPerSec(1) during(10)),
    getShopDetail.inject(constantUsersPerSec(100) during (10))
  ).protocols(httpConf)
}
