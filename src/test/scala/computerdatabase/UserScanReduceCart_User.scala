...
class UserScanReduceCart_User extends Simulation {
...
//  println("Data Source Length:" + feeder2.records.length)
  ...
  object addReduceTest {
    val test = scenario("user_reduceCart") // A scenario is a chain of requests and pauses
      .feed(feeder)
      .feed(feeder2)
...
//            }
            .doIf(session => session.contains("pid") && session.contains("specId")) {
              exec(http("add_cart1").post("ja/user/v1/cart/10/${pid}/${specId}/add")
                ...
                exec(http("add_cart2").post("ja/user/v1/cart/10/${pid}/${specId}/reduce")
                  .headers(Map("token" -> "${token_w}"))
                  .body(StringBody("""{"tableNo":"5"}"""))
                  .asJSON.check(status.is(200) ,jsonPath("$.statusCode").is(0.toString()))
                )
  ...
  val user_reduceCart = scenario("user_reduceCart").exec(addReduceTest.test)
  setUp(
    //    notice.inject(constantUsersPerSec(1) during(10)),
    user_reduceCart.inject(constantUsersPerSec(20) during (10))
  ).protocols(httpConf)
}
