...
class UserCommitOrder_User extends Simulation {
...
//  println("Data Source Length:" + feeder2.records.length)
  ...
  object commitOrderTest {
    val test = scenario("commitOrder") // A scenario is a chain of requests and pauses
      .feed(feeder)
      .feed(feeder2)
...
//      )
  ...
  val commitOrder = scenario("commitOrder").exec(commitOrderTest.test)
  setUp(
    //    notice.inject(constantUsersPerSec(1) during(10)),
    commitOrder.inject(constantUsersPerSec(10) during (10))
  ).protocols(httpConf)
}
